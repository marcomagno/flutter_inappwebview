...
public class FlutterWebViewController: NSObject, FlutterPlatformView {
  ...
  public var webView: InAppWebView?
  var viewId: Any = 0
  ...
  init(
    registrar: FlutterPluginRegistrar, withFrame frame: CGRect, viewIdentifier viewId: Any,
    params: NSDictionary
  ) {
    super.init()
    ...
    channel = FlutterMethodChannel(
      name: "com.pichillilorenzo/flutter_inappwebview_" + String(describing: viewId),
    ...
    let pullToRefreshLayoutChannel = FlutterMethodChannel(
      name: "com.pichillilorenzo/flutter_inappwebview_pull_to_refresh_"
    ...
    let pullToRefreshControl = PullToRefreshControl(
      channel: pullToRefreshLayoutChannel, options: pullToRefreshOptions)
  ...
  func load(
    initialUrlRequest: [String: Any?]?, initialFile: String?, initialData: [String: String]?
  ) {
    if let initialFile = initialFile {
      ...
      var allowingReadAccessToURL: URL? = nil
      if let allowingReadAccessTo = webView?.options?.allowingReadAccessTo,
        let url = urlRequest.url, url.scheme == "file"
      {
        allowingReadAccessToURL = URL(string: allowingReadAccessTo)
        if allowingReadAccessToURL?.scheme != "file" {
          allowingReadAccessToURL = nil
