...
public class InAppWebView: WKWebView, UIScrollViewDelegate, WKUIDelegate, WKNavigationDelegate,
  WKScriptMessageHandler, UIGestureRecognizerDelegate, PullToRefreshDelegate
{
  ...
  public var primaryNavigationPolicyHandler:
    ((WKWebView, WKNavigationAction, @escaping (WKNavigationActionPolicy) -> Void) -> Void)?

  init(
    frame: CGRect, configuration: WKWebViewConfiguration, contextMenu: [String: Any]?,
    channel: FlutterMethodChannel?, userScripts: [UserScript] = []
  ) {
      ...
      if let menuItems = menu["menuItems"] as? [[String: Any]] {
        for menuItem in menuItems {
          ...
          if !self.responds(to: Selector(targetMethodName)) {
            ...
            class_addMethod(
              InAppWebView.self, Selector(targetMethodName), swizzledImplementation, nil)
      ...
      if let menu = contextMenu {
        ...
        if let contextMenuOptionsMap = menu["options"] as? [String: Any?] {
          ...
          if !action.description.starts(with: "onContextMenuActionItemClicked-")
            && contextMenuOptions.hideDefaultSystemContextMenuItems
          {
            return false
          }
        }
  ...
  public func prepare() {
    ...
    scrollView.addObserver(
      self, forKeyPath: #keyPath(UIScrollView.contentOffset), options: [.new, .old], context: nil)
    scrollView.addObserver(
    ...
    if let options = options {
      ...
      if #available(iOS 11.0, *) {
        ...
        scrollView.contentInsetAdjustmentBehavior =
          UIScrollView.ContentInsetAdjustmentBehavior.init(
            rawValue: options.contentInsetAdjustmentBehavior)!
      }
      ...
      scrollView.isScrollEnabled =
        !(options.disableVerticalScroll && options.disableHorizontalScroll)
    ...
    if let options = options {
      ...
      if #available(iOS 13.0, *) {
        ...
        configuration.defaultWebpagePreferences.preferredContentMode =
          WKWebpagePreferences.ContentMode(rawValue: options.preferredContentMode)!
      }
  ...
  public static func preWKWebViewConfiguration(options: InAppWebViewOptions?)
    -> WKWebViewConfiguration
  {
    ...
    if let options = options {
      ...
      configuration.selectionGranularity = WKSelectionGranularity.init(
        rawValue: options.selectionGranularity)!

      if options.allowUniversalAccessFromFileURLs {
        configuration.setValue(
          options.allowUniversalAccessFromFileURLs, forKey: "allowUniversalAccessFromFileURLs")
      }

      if options.allowFileAccessFromFileURLs {
        configuration.preferences.setValue(
          options.allowFileAccessFromFileURLs, forKey: "allowFileAccessFromFileURLs")
      }
        ...
        if !options.applicationNameForUserAgent.isEmpty {
          if let applicationNameForUserAgent = configuration.applicationNameForUserAgent {
            configuration.applicationNameForUserAgent =
              applicationNameForUserAgent + " " + options.applicationNameForUserAgent
          }
        }
      }
      ...
      if #available(iOS 10.0, *) {
        ...
        configuration.mediaTypesRequiringUserActionForPlayback =
          options.mediaPlaybackRequiresUserGesture ? .all : []
      } else {
  ...
  func setOptions(newOptions: InAppWebViewOptions, newOptionsMap: [String: Any]) {
    ...
    if #available(iOS 13.0, *) {
      if newOptionsMap["applePayAPIEnabled"] != nil
    ...
    if newOptionsMap["transparentBackground"] != nil
      && options?.transparentBackground != newOptions.transparentBackground
    {
    ...
    if newOptionsMap["disallowOverScroll"] != nil
      && options?.disallowOverScroll != newOptions.disallowOverScroll
    {
      if responds(to: #selector(getter: scrollView)) {
    ...
    if #available(iOS 9.0, *) {
      if newOptionsMap["incognito"] != nil && options?.incognito != newOptions.incognito
        && newOptions.incognito
      {
        configuration.websiteDataStore = WKWebsiteDataStore.nonPersistent()
      } else if newOptionsMap["cacheEnabled"] != nil
        && options?.cacheEnabled != newOptions.cacheEnabled && newOptions.cacheEnabled
      {
        configuration.websiteDataStore = WKWebsiteDataStore.default()
      }
    }
    ...
    if #available(iOS 11.0, *) {
      if newOptionsMap["sharedCookiesEnabled"] != nil
        && options?.sharedCookiesEnabled != newOptions.sharedCookiesEnabled
        && newOptions.sharedCookiesEnabled
      {
      ...
      if newOptionsMap["accessibilityIgnoresInvertColors"] != nil
        && options?.accessibilityIgnoresInvertColors != newOptions.accessibilityIgnoresInvertColors
      {
        accessibilityIgnoresInvertColors = newOptions.accessibilityIgnoresInvertColors
      }
      if newOptionsMap["contentInsetAdjustmentBehavior"] != nil
        && options?.contentInsetAdjustmentBehavior != newOptions.contentInsetAdjustmentBehavior
      {
        scrollView.contentInsetAdjustmentBehavior =
          UIScrollView.ContentInsetAdjustmentBehavior.init(
            rawValue: newOptions.contentInsetAdjustmentBehavior)!
      }
    }

    if newOptionsMap["enableViewportScale"] != nil
      && options?.enableViewportScale != newOptions.enableViewportScale
    {
    ...
    if newOptionsMap["useOnLoadResource"] != nil
      && options?.useOnLoadResource != newOptions.useOnLoadResource
    {
      if let applePayAPIEnabled = options?.applePayAPIEnabled, !applePayAPIEnabled {
    ...
    if newOptionsMap["useShouldInterceptAjaxRequest"] != nil
      && options?.useShouldInterceptAjaxRequest != newOptions.useShouldInterceptAjaxRequest
    {
      if let applePayAPIEnabled = options?.applePayAPIEnabled, !applePayAPIEnabled {
        enablePluginScriptAtRuntime(
    ...
    if newOptionsMap["useShouldInterceptFetchRequest"] != nil
      && options?.useShouldInterceptFetchRequest != newOptions.useShouldInterceptFetchRequest
    {
      if let applePayAPIEnabled = options?.applePayAPIEnabled, !applePayAPIEnabled {
        enablePluginScriptAtRuntime(
    ...
    if newOptionsMap["mediaPlaybackRequiresUserGesture"] != nil
      && options?.mediaPlaybackRequiresUserGesture != newOptions.mediaPlaybackRequiresUserGesture
    {
      if #available(iOS 10.0, *) {
        configuration.mediaTypesRequiringUserActionForPlayback =
          (newOptions.mediaPlaybackRequiresUserGesture) ? .all : []
      } else {
        // Fallback on earlier versions
    ...
    if newOptionsMap["allowsInlineMediaPlayback"] != nil
      && options?.allowsInlineMediaPlayback != newOptions.allowsInlineMediaPlayback
    {
      configuration.allowsInlineMediaPlayback = newOptions.allowsInlineMediaPlayback
    }

    if newOptionsMap["suppressesIncrementalRendering"] != nil
      && options?.suppressesIncrementalRendering != newOptions.suppressesIncrementalRendering
    {
      configuration.suppressesIncrementalRendering = newOptions.suppressesIncrementalRendering
    }

    if newOptionsMap["allowsBackForwardNavigationGestures"] != nil
      && options?.allowsBackForwardNavigationGestures
        != newOptions.allowsBackForwardNavigationGestures
    {
      allowsBackForwardNavigationGestures = newOptions.allowsBackForwardNavigationGestures
    }

    if newOptionsMap["javaScriptCanOpenWindowsAutomatically"] != nil
      && options?.javaScriptCanOpenWindowsAutomatically
        != newOptions.javaScriptCanOpenWindowsAutomatically
    {
      configuration.preferences.javaScriptCanOpenWindowsAutomatically =
        newOptions.javaScriptCanOpenWindowsAutomatically
    }

    if newOptionsMap["minimumFontSize"] != nil
      && options?.minimumFontSize != newOptions.minimumFontSize
    {
      configuration.preferences.minimumFontSize = CGFloat(newOptions.minimumFontSize)
    }

    if newOptionsMap["selectionGranularity"] != nil
      && options?.selectionGranularity != newOptions.selectionGranularity
    {
      configuration.selectionGranularity = WKSelectionGranularity.init(
        rawValue: newOptions.selectionGranularity)!
    }

    if #available(iOS 10.0, *) {
      if newOptionsMap["ignoresViewportScaleLimits"] != nil
        && options?.ignoresViewportScaleLimits != newOptions.ignoresViewportScaleLimits
      {
        configuration.ignoresViewportScaleLimits = newOptions.ignoresViewportScaleLimits
      }

      if newOptionsMap["dataDetectorTypes"] != nil
        && options?.dataDetectorTypes != newOptions.dataDetectorTypes
      {
        var dataDetectorTypes = WKDataDetectorTypes.init(rawValue: 0)
    ...
    if #available(iOS 13.0, *) {
      if newOptionsMap["isFraudulentWebsiteWarningEnabled"] != nil
        && options?.isFraudulentWebsiteWarningEnabled
          != newOptions.isFraudulentWebsiteWarningEnabled
      {
        configuration.preferences.isFraudulentWebsiteWarningEnabled =
          newOptions.isFraudulentWebsiteWarningEnabled
      }
      if newOptionsMap["preferredContentMode"] != nil
        && options?.preferredContentMode != newOptions.preferredContentMode
      {
        configuration.defaultWebpagePreferences.preferredContentMode =
          WKWebpagePreferences.ContentMode(rawValue: newOptions.preferredContentMode)!
      }
      if newOptionsMap["automaticallyAdjustsScrollIndicatorInsets"] != nil
        && options?.automaticallyAdjustsScrollIndicatorInsets
          != newOptions.automaticallyAdjustsScrollIndicatorInsets
      {
        scrollView.automaticallyAdjustsScrollIndicatorInsets =
          newOptions.automaticallyAdjustsScrollIndicatorInsets
      }
    ...
    if newOptionsMap["disableVerticalScroll"] != nil
      && options?.disableVerticalScroll != newOptions.disableVerticalScroll
    {
      scrollView.showsVerticalScrollIndicator = !newOptions.disableVerticalScroll
    }
    if newOptionsMap["disableHorizontalScroll"] != nil
      && options?.disableHorizontalScroll != newOptions.disableHorizontalScroll
    {
      scrollView.showsHorizontalScrollIndicator = !newOptions.disableHorizontalScroll
    }

    if newOptionsMap["verticalScrollBarEnabled"] != nil
      && options?.verticalScrollBarEnabled != newOptions.verticalScrollBarEnabled
    {
      scrollView.showsVerticalScrollIndicator = newOptions.verticalScrollBarEnabled
    }
    if newOptionsMap["horizontalScrollBarEnabled"] != nil
      && options?.horizontalScrollBarEnabled != newOptions.horizontalScrollBarEnabled
    {
      scrollView.showsHorizontalScrollIndicator = newOptions.horizontalScrollBarEnabled
    }

    if newOptionsMap["isDirectionalLockEnabled"] != nil
      && options?.isDirectionalLockEnabled != newOptions.isDirectionalLockEnabled
    {
      scrollView.isDirectionalLockEnabled = newOptions.isDirectionalLockEnabled
    }

    if newOptionsMap["decelerationRate"] != nil
      && options?.decelerationRate != newOptions.decelerationRate
    {
      scrollView.decelerationRate = Util.getDecelerationRate(type: newOptions.decelerationRate)
    }
    if newOptionsMap["alwaysBounceVertical"] != nil
      && options?.alwaysBounceVertical != newOptions.alwaysBounceVertical
    {
      scrollView.alwaysBounceVertical = newOptions.alwaysBounceVertical
    }
    if newOptionsMap["alwaysBounceHorizontal"] != nil
      && options?.alwaysBounceHorizontal != newOptions.alwaysBounceHorizontal
    {
    ...
    if newOptionsMap["isPagingEnabled"] != nil
      && options?.isPagingEnabled != newOptions.isPagingEnabled
    {
      scrollView.scrollsToTop = newOptions.isPagingEnabled
    }
    if newOptionsMap["maximumZoomScale"] != nil
      && options?.maximumZoomScale != newOptions.maximumZoomScale
    {
      scrollView.maximumZoomScale = CGFloat(newOptions.maximumZoomScale)
    }
    if newOptionsMap["minimumZoomScale"] != nil
      && options?.minimumZoomScale != newOptions.minimumZoomScale
    {
      scrollView.minimumZoomScale = CGFloat(newOptions.minimumZoomScale)
    }

    if #available(iOS 9.0, *) {
      if newOptionsMap["allowsLinkPreview"] != nil
        && options?.allowsLinkPreview != newOptions.allowsLinkPreview
      {
        allowsLinkPreview = newOptions.allowsLinkPreview
      }
      if newOptionsMap["allowsAirPlayForMediaPlayback"] != nil
        && options?.allowsAirPlayForMediaPlayback != newOptions.allowsAirPlayForMediaPlayback
      {
        configuration.allowsAirPlayForMediaPlayback = newOptions.allowsAirPlayForMediaPlayback
      }
      if newOptionsMap["allowsPictureInPictureMediaPlayback"] != nil
        && options?.allowsPictureInPictureMediaPlayback
          != newOptions.allowsPictureInPictureMediaPlayback
      {
        configuration.allowsPictureInPictureMediaPlayback =
          newOptions.allowsPictureInPictureMediaPlayback
      }
      if newOptionsMap["applicationNameForUserAgent"] != nil
        && options?.applicationNameForUserAgent != newOptions.applicationNameForUserAgent
        && newOptions.applicationNameForUserAgent != ""
      {
        configuration.applicationNameForUserAgent = newOptions.applicationNameForUserAgent
      }
      if newOptionsMap["userAgent"] != nil && options?.userAgent != newOptions.userAgent
        && newOptions.userAgent != ""
      {
        customUserAgent = newOptions.userAgent
      }
    }

    if newOptionsMap["allowUniversalAccessFromFileURLs"] != nil
      && options?.allowUniversalAccessFromFileURLs != newOptions.allowUniversalAccessFromFileURLs
    {
      configuration.setValue(
        newOptions.allowUniversalAccessFromFileURLs, forKey: "allowUniversalAccessFromFileURLs")
    }

    if newOptionsMap["allowFileAccessFromFileURLs"] != nil
      && options?.allowFileAccessFromFileURLs != newOptions.allowFileAccessFromFileURLs
    {
      configuration.preferences.setValue(
        newOptions.allowFileAccessFromFileURLs, forKey: "allowFileAccessFromFileURLs")
    }
    ...
    if newOptionsMap["javaScriptEnabled"] != nil
      && options?.javaScriptEnabled != newOptions.javaScriptEnabled
    {
    ...
    if #available(iOS 14.0, *) {
      ...
      if newOptionsMap["limitsNavigationsToAppBoundDomains"] != nil
        && options?.limitsNavigationsToAppBoundDomains
          != newOptions.limitsNavigationsToAppBoundDomains
      {
        configuration.limitsNavigationsToAppBoundDomains =
          newOptions.limitsNavigationsToAppBoundDomains
      }
    ...
    scrollView.isScrollEnabled =
      !(newOptions.disableVerticalScroll && newOptions.disableHorizontalScroll)
      ...
      if let alreadyLoaded = alreadyLoaded as? Bool, alreadyLoaded {
        ...
        if #available(iOS 14.0, *), pluginScript.requiredInAllContentWorlds {
          for contentWorld in self.configuration.userContentController.contentWorlds {
            self.evaluateJavaScript(
              enableSource, frame: nil, contentWorld: contentWorld, completionHandler: nil)
          }
        } else {
        ...
        if #available(iOS 14.0, *), pluginScript.requiredInAllContentWorlds {
          for contentWorld in self.configuration.userContentController.contentWorlds {
            self.evaluateJavaScript(
              pluginScript.source, frame: nil, contentWorld: contentWorld, completionHandler: nil)
  ...
  public func injectDeferredObject(
    ...
    evaluateJavaScript(jsToInject) { (value, error) in
      ...
      if let error = error {
        ...
        let errorMessage =
          userInfo["WKJavaScriptExceptionMessage"] ?? userInfo["NSLocalizedDescription"] as? String
  ...
  public func callAsyncJavaScript(
    ...
    callAsyncJavaScript(jsToInject, arguments: arguments, frame: nil, contentWorld: contentWorld) {
      (evalResult) in
      guard let completionHandler = completionHandler else {
        return
      }
  ...
  public func callAsyncJavaScript(
    ...
    jsToInject =
      ...
      .replacingOccurrences(
        of: PluginScriptsUtil.VAR_FUNCTION_ARGUMENTS_OBJ, with: Util.JSONStringify(value: arguments)
      )
  ...
  public func webView(
    ...
    if navigationAction.request.url != nil {
      ...
      if let useShouldOverrideUrlLoading = options?.useShouldOverrideUrlLoading,
            ...
            decisionHandlerCrashAvoider.run = { () -> Void in
              ...
              if result is FlutterError {
                ...
                if let r = result {
                  ...
                  let navigationActionPolicy =
                    WKNavigationActionPolicy.init(
                      rawValue: action ?? WKNavigationActionPolicy.cancel.rawValue)
  ...
  public func webView(
      ...
      let response = navigationResponse.response as? HTTPURLResponse
    {
      if response.statusCode >= 400 {
        onLoadHttpError(
          url: response.url?.absoluteString, statusCode: response.statusCode, description: "")
      }
    }
              ...
              if let r = result {
                ...
                case 1:
                  ...
                  let persistence =
                    ...
                    ? URLCredential.Persistence.permanent : URLCredential.Persistence.forSession
                ...
                case 2:
                  if InAppWebView.credentialsProposed.count == 0,
                    let credentialStore = CredentialDatabase.credentialStore
                  {
  ...
  func createAlertDialog(
    ...
    guard
      let presentingViewController = inAppBrowserDelegate != nil
        ? inAppBrowserDelegate as? InAppBrowserWebViewController : window?.rootViewController
    else {
      completionHandler()
      return
    }
        ...
        decisionHandlerCrashAvoider.run = { () -> Void in
          ...
          if result is FlutterError {
            ...
            self.createAlertDialog(
              message: message, responseMessage: nil, confirmButtonTitle: nil,
              completionHandler: completionHandler)
          } else {
        ...
        decisionHandlerCrashAvoider.run = { () -> Void in
          ...
          if result is FlutterError {
            ...
            self.createConfirmDialog(
              message: message, responseMessage: nil, confirmButtonTitle: nil,
              cancelButtonTitle: nil, completionHandler: completionHandler)
          } else {
            let response: [String: Any]
            var responseMessage: String?
  ...
  func createPromptDialog(
    message: String, defaultValue: String?, responseMessage: String?, confirmButtonTitle: String?,
    cancelButtonTitle: String?, value: String?, completionHandler: @escaping (String?) -> Void
  ) {
    let dialogMessage =
      responseMessage != nil && !responseMessage!.isEmpty ? responseMessage : message
    let okButton =
  ...
  public func webView(
    ...
    onJsPrompt(
      frame: frame, message: message, defaultValue: defaultValue,
      result: { (result) -> Void in
        decisionHandlerCrashAvoider.run = { () -> Void in
          if completionHandlerCalled {
            return
          ...
          if result is FlutterError {
            ...
            self.createPromptDialog(
              message: message, defaultValue: defaultValue, responseMessage: nil,
              confirmButtonTitle: nil, cancelButtonTitle: nil, value: nil,
              completionHandler: completionHandler)
          } else {
            let response: [String: Any]
            var responseMessage: String?
            ...
            self.createPromptDialog(
              ...
              confirmButtonTitle: confirmButtonTitle, cancelButtonTitle: cancelButtonTitle,
              value: value, completionHandler: completionHandler)
          }
        }
        decisionHandlerCrashAvoider.run()
      })
  ...
  public func onScrollChanged(startedByUser: Bool, oldContentOffset: CGPoint?) {
    ...
    let overScrolledHorizontally =
      lastScrollX < 0 || lastScrollX > (scrollView.contentSize.width - scrollView.frame.size.width)
  ...
  public func webView(
    ...
    let windowWebView = InAppWebView(
      frame: CGRect.zero, configuration: configuration, contextMenu: nil, channel: nil)
    ...
    channel?.invokeMethod(
      "onCreateWindow", arguments: arguments,
      result: { (result) -> Void in
        if result is FlutterError {
          print((result as! FlutterError).message ?? "")
  ...
  {
    channel?.invokeMethod(
      "shouldOverrideUrlLoading", arguments: navigationAction.toMap(), result: result)
  }

  public func onNavigationResponse(navigationResponse: WKNavigationResponse, result: FlutterResult?)
  {
    channel?.invokeMethod(
      "onNavigationResponse", arguments: navigationResponse.toMap(), result: result)
  }

  public func onReceivedHttpAuthRequest(
    challenge: URLAuthenticationChallenge, result: FlutterResult?
  ) {
    channel?.invokeMethod(
      "onReceivedHttpAuthRequest",
  ...
  public func onJsPrompt(
    frame: WKFrameInfo, message: String, defaultValue: String?, result: FlutterResult?
  ) {
    let arguments: [String: Any?] = [
      "url": frame.request.url?.absoluteString,
      "message": message,
  ...
  public func onCallJsHandler(handlerName: String, _callHandlerID: Int64, args: String) {
    ...
    channel?.invokeMethod(
      "onCallJsHandler", arguments: arguments,
      result: { (result) -> Void in
        if result is FlutterError {
          print((result as! FlutterError).message ?? "")
        } else if (result as? NSObject) == FlutterMethodNotImplemented {
...
""",
    ...
    if message.name.starts(with: "console") {
        ...
        where: ({ ($0.jsObjectName == jsObjectName) }))
      {
        ...
        if let scheme = scheme, !scheme.isEmpty, let host = host, !host.isEmpty {
          sourceOrigin = URL(
            string: "\(scheme)://\(host)\(port != nil && port != 0 ? ":" + String(port!) : "")")
        }
        webMessageListener.onPostMessage(
          message: messageData, sourceOrigin: sourceOrigin, isMainFrame: isMainFrame)
      }
    }
  }
  ...
  public func clearMatches(completionHandler: ((Any?, Error?) -> Void)?) {
    evaluateJavaScript(
      "window.\(JAVASCRIPT_BRIDGE_NAME)._clearMatches();", completionHandler: completionHandler)
  }
  ...
  public func printCurrentPage(
    printCompletionHandler: ((_ completed: Bool, _ error: Error?) -> Void)?
  ) {
    ...
    let completionHandler: UIPrintInteractionController.CompletionHandler = {
      (printController, completed, error) in
      if !completed {
        if let e = error {
  ...
  public func addWebMessageListener(webMessageListener: WebMessageListener) throws {
    if webMessageListeners.map({ ($0.jsObjectName) }).contains(webMessageListener.jsObjectName) {
      throw NSError(
        domain: "jsObjectName \(webMessageListener.jsObjectName) was already added.", code: 0)
    }
